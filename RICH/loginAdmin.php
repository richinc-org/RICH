                  ...
                  <div class="container-fluid">
                    ...
                    <div class="centerit">
                      <ul class="site-menu js-clone-nav d-none d-lg-block">
                        ...
                        <li class="has-children">
                          <a href="graduate-testimonials.html">Success Stories</a>
                          <ul class="dropdown arrow-top">
					...
					<li class="has-children">
						...
						<ul class="dropdown arrow-top">
							<li><a href="podcasts.html">Reach For Success</a></li>
						</ul>
					</li>
          ...
          <div class="shapely-content">
            ...
            <div class="container" id="LoginForm">
              <form action="" method="POST">
                ...
                <h3 style="text-align: center; color: white;">RICH ADMINS</h3>
                <div class="form-group">
                  <label style="float:left;" for="userName">Username</label>
                  <input type="email" class="form-control" id="userEmail" name="logSAemail" required></input>
                </div>
                <div class="form-group">
                  <label style="float:left;" for="userPass"> Password</label>
                  <input type="password" class="form-control" id="userPass" name="logSApass" required></input>
                </div>
                ...
                <div class="form-group">
                  <a class="formlink" href="login.php"> Go To Student Account </a><br><br>
                </div>
                <div class="buttoncontainer">
                  <button type="submit" name="logSAsub"> Sign in </button>
                </div>

              <?php
                if (isset($_POST)) {
                  if (isset($_POST['logSAsub'])) {
                    $salogName = htmlspecialchars($_POST['logSAemail']);
                    $salogPass = htmlspecialchars($_POST['logSApass']);
                    $db = new Database;
                    $conn = $db->connect();

                    $sql = "SELECT EmailAddress, Pass, Uid, SchoolUid, FirstName, LastName FROM SchoolAdmins WHERE EmailAddress = '$salogName';";
                    ...
                    if ($result) {
                      ...
                      if ($queryData > 0){
                        while ($row = mysqli_fetch_assoc($result)) {
                          $SApwdHashed = $row['Pass'];
                          if ($salogName == $row['EmailAddress']){
                            if(password_verify($salogPass, $SApwdHashed)){
                              session_start();
                              $_SESSION['AdminuserName'] = $salogName;
                              $_SESSION['Adminuid'] = $row['Uid'];
                              $_SESSION['Adminschool'] = $row['SchoolUid'];
            ?>
                              <script type="text/javascript">
                                window.location.href = "SchoolA/roster.php";
                              </script>
            <?php
                            }
                            else{
