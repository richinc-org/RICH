<?php 
  ...
  function userExists($conn, $username, $email){
    $result;
    $sql = "SELECT * FROM Students WHERE EmailAddress = '$username' OR EmailAddress = '$email';";
    $DB = new Database;
  ...
  function evaluate($first, $last, $birthday, $gender, $grade, $uprinciple, $email, $password, $password_match, $scid, $conn){
    if(userExists($conn, $username, $email) == "true"){
      ...
      if(checkPasswordLength($password) == "true"){
        ...
        if (checkMatch($password, $password_match) == "true") {
          ...
          createuser($first, $last, $birthday, $gender, $grade, $uprinciple, $email, $password, $scid, $conn);
        }
      }
    }
  }

  function createuser($first, $last, $birthday, $gender, $grade, $uprinciple, $email, $password, $scid, $conn){
    $status = "active";
    $userid = createuid();
    $DB = new Database;
    $Check = new Uid();
    ...
    $sql = "INSERT INTO Students (Uid, SchoolUid, FirstName, LastName, DOB, Gender, GradeLevel, Principle, EmailAddress, Pass, Status) VALUES ('$userid', '$scid', '$first', '$last', '$birthday', '$gender', '$grade', '$uprinciple', '$email', '$hashedPwd', '$status');";

    
    $DB ->save($sql);

    header("location: register.php?error=none&sid=$scid");
    die();
  }
  ...
  if (isset($_POST)) {
    if (isset($_POST['btnsubmit'])) {
      ...
      evaluate($first, $last, $birthday, $gender, $grade, $uprinciple, $email, $password, $password_match, $scid, $conn);
    }   
  }
  ...
  <h1 class= "text-center"> Student Registration </h1><br>
    ...
    <div id = "studentForm">
      ...
      <?php 
        if (isset($_GET)) {
          if (htmlspecialchars($_GET['error']) == 'none') {
            echo "<p id='successlog'>Account Created Successfully.</p>";
            echo "<a href= 'https://www.richinc.org/RICH/beforeregister.php'><button type='button' style='width:50%;'> Change Schools </button></a>";
            echo "<br><br>";
          }
      ...
      <div class="form-group">
        ...
        <input type = "text" name="userfname" class="form-control" id="firstName" required></input>
      </div>

      <div class="form-group">
        <label style="float:left;" for="lastName"> Last Name (required):</label>
        <input type = "text" name="userlname" class="form-control" id="lastName" required></input>
      </div>

      <div class="form-group">
        <label style="float:left;" for="dateOfBirth">Date of Birth:</label>
        <input type = "text" name="userdob" class="form-control" id="dob" placeholder="YYYY-MM-DD"required></input>
      </div>

      <div class="formgroup">
        <label style="float:left;" for="gender"> Gender: </label>
        <div class ="form-group">
          <select onchange = "checkGender()" name="usergender" class="form-control" id="gender">
      ...
      <div class="form-group">
        <label style="float:left;" for="gradeLevel">Grade Level:</label>
        <input type = "text" name="usergrade" class="form-control" id="gradeLevel" required></input>
      </div>

      <div class="form-group">
        <label style="float:left;" for="purpose">Which principle would you like to focus on the most?</label>
        <div class="form-group">
          <select onchange = "checkPrinciple()" name="userprinciple" class="form-control" id="principles">
            <option disabled selected> Choose One of the Four </option>
            <option>I matter</option>
            <option>I am responsible for my behavior</option>
            <option>I am considerate of classmates and others</option>
            <option>I use thinking strategies for school success</option>
          </select>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <input type = "email" name="useremail" class="form-control" id="email" required></input>
      </div>

      <div class="form-group">
        <label style="float:left;" for="password">Password:</label>
        <input type = "password" name="userpassword" class="form-control" id="pass" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" oninvalid="this.setCustomValidity('Password Must Contain At Least One of Each: Uppercase, Lowercase and a Number')" required></input>
      </div>

      <div class="form-group">
        <label style="float:left;" for="passwordC">Confirm Password:</label>
        <input type = "password" name="usercpassword" class="form-control" id="passC" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" oninvalid="this.setCustomValidity('Password Must Contain At Least One of Each: Uppercase, Lowercase and a Number')" required></input>
      </div>

      <div>
        <button type="submit" name="btnsubmit" value="submit" style="width:100%;"> Create Account </button>
      </div>
    </div>
</form>
