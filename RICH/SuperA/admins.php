<?php
  ...
  if (isset($_POST)) {
    if (isset($_POST['submit'])) {
      ...
      $sqltt = "UPDATE SchoolAdmins SET SchoolUid = '$x', FirstName = '$newfn', LastName = '$newln', EmailAddress = '$newea' WHERE Uid = '$stuuid';";
      $db->save($sqltt);
  ...
  <div class="site-wrap">
        ...
        <div class="site-navbar bg-light">
          <div class="py-1">
            ...
            <?php
                              ...
                              $sqlb = "SELECT * FROM Schools";
                              $resultb = $db->read($sqlb);

                              if ($resultb) {
                                $queryDatab = mysqli_num_rows($resultb);

                                if ($queryDatab > 0){
                                  while ($rowb = mysqli_fetch_assoc($resultb)) {
            ?>
                                    <option value="<?php echo htmlspecialchars($rowb['Uid']); ?>">
                                      <?php echo htmlspecialchars($rowb['SchoolName']); ?>
                                    </option>
            <?php
                                  }
                                }
    ...
    <div style="padding-bottom: 350px;" class="text-center">
      ...
      ?>
        ...
        <?php
                        $sql = "SELECT * FROM SchoolAdmins WHERE Uid = '$studentID';";
                        $result = $db->read($sql);
        ...
        <?php
              ...
              if (htmlspecialchars(($_GET['a']) == 'abl')) {
                $db = new Database;
                $conn = $db->connect();

                $studentID = htmlspecialchars($_GET['id']);

                $sqlcom = "UPDATE SchoolAdmins SET Status = 'active' WHERE Uid = '$studentID';";
                $db->save($sqlcom);
            ?>
            ...
            <?php
              ...
              if (htmlspecialchars(($_GET['a']) == 'dis')) {
                $db = new Database;
                $conn = $db->connect();

                $studentID = htmlspecialchars($_GET['id']);

                $sqlcom = "UPDATE SchoolAdmins SET Status = 'disabled' WHERE Uid = '$studentID';";
                $db->save($sqlcom);
            ?>
                <script type="text/javascript">
                  window.location.href = "admins.php";
            ...
            <?php
              ...
              if (htmlspecialchars(($_GET['a']) == 'del')) {
                ...
                $sqlcom = "DELETE FROM SchoolAdmins WHERE Uid = '$studentID';";
                $db->save($sqlcom);
            ?>
                <script type="text/javascript">
                  window.location.href = "admins.php";
    ...
    <?php
              ...
              if (isset($_POST['filter'])) {
                $school = addslashes($_POST['schools']);
                if ($school == "all") {
                  $sqlthree = "SELECT * FROM SchoolAdmins";
                }
                else{
                  $sqlthree = "SELECT * FROM SchoolAdmins WHERE SchoolUid = '$school';";
                }
              }
              else{
                $sqlthree = "SELECT * FROM SchoolAdmins";
              ...
              if ($resulthree) {
                ...
                if ($qDthree > 0){
                  ...
                  if (isset($_POST['filter'])) {
                    $school = addslashes($_POST['schools']);
                    if ($school == "all") {
                      $sqlfor = "SELECT * FROM SchoolAdmins LIMIT " . $first_result . "," . $resultsperpage;
                    }
                    else{
                      $sqlfor = "SELECT * FROM SchoolAdmins WHERE SchoolUid = '$school' LIMIT " . $first_result . "," . $resultsperpage;
                    }
                  }
                  else{
                    $sqlfor = "SELECT * FROM SchoolAdmins LIMIT " . $first_result . "," . $resultsperpage;
                  ...
                  if ($resulthree) {
                    ...
                    if ($qDfor > 0){
                      ...
                      for($page=$number_of_pages; $page>0; $page--){
                        echo "<a id='right' href='admins.php?page=" . $page . "'>" . $page . "</a> ";
                      }
    ?>
          </div>
    <?php
    ...
    <?php                 
        ...
        ?> 
                            ...
                            <td name = 'ca'>
                              <a href="fullview.php?said=<?php echo htmlspecialchars($rowC['Uid']); ?>"><button class="action"><i class="fas fa-eye"></i></button></a>

                              <a href="admins.php?a=e&id=<?php echo htmlspecialchars($rowC['Uid']); ?>"><button class="action"><i class="fas fa-edit"></i></button></a>

                              <?php 
                                if (htmlspecialchars($rowC['Status']) == 'disabled') {
                              ?>
                                  <a href="admins.php?a=abl&id=<?php echo htmlspecialchars($rowC['Uid']); ?>">
                                    <button class="action"><i class='fas fa-user'></i></button>
                                  </a>
                              <?php
                                }
                                else{
                              ?>
                                  <a href="admins.php?a=dis&id=<?php echo htmlspecialchars($rowC['Uid']); ?>">
                                    <button class="action"><i class='fas fa-user-slash'></i></button>
                                  </a>
                              <?php
                                }
                              ?>

                              <a href="admins.php?a=del&id=<?php echo htmlspecialchars($rowC['Uid']); ?>"><button class="action"><i class="fas fa-trash"></i></button></a>
                            </td>
                          </tr>
        <?php
                          $count++;
